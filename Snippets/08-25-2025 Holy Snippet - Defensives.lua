...
A[3] = function(icon, isMulti)
    ...
    local function HealingRotation(unitID) 
        ...
        if not Action.InstanceInfo.isRated or Action.InstanceInfo.isRated and (Unit(unitID):HealthPercent() >= 30 or Unit(unitID):HealthPercent() < 30 and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady()) then
                ...
                for i = 1, #getmembersAll do  
                    ...
                    then
                        ...
                        if debuff275014Present then
                            ...
                            if Unit(getmembersAll[i].Unit):HasDeBuffs(275014) > 2 and (not Unit(getmembersAll[i].Unit):Role("TANK") or (HealingEngine.GetDeBuffsCount(275014, 18) == 1 and Unit(getmembersAll[i].Unit):Role("TANK") and Unit(getmembersAll[i].Unit):HasDeBuffs(275014) > 15)) then
                                HealingEngine.SetTarget(getmembersAll[i].Unit, 0.5)
                            end
                        else
            ...
            useDispel and Unit(unitID):HasDeBuffs(342938) == 0 and (Unit(unitID):HasDeBuffs(275014) == 0 or (Unit(unitID):HasDeBuffs(275014) > 2 and not Unit(unitID):Role("TANK")) or (HealingEngine.GetDeBuffsCount(275014, 18) == 1 and Unit(unitID):Role("TANK") and (Unit(unitID):HasDeBuffs(275014) > 15))) and
            (
                -- MouseOver
                (
    ...
    local function DamageRotation(unitID)
        ...
        if A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp" or inCombat then
            ...
            local function CheckEnemyCastInterrupt()
                local spellIDsToCheck = {
                    293827, -- Giga-Wallop
                    330586, -- Devour Flesh
                    427342, -- Defend
            ...
            if useCC and A.BlindingLight:IsReady() and CheckEnemyCastInterrupt() then
