...
local DispelRules = {
  [1240097] = { minRemain = 2,  maxRemain = 20, minStack = 0 },          -- “let it tick” until <20s
  [461487]  = { minRemain = 2,  maxRemain = 6,  minStack = 0 },
  [473713]  = { minRemain = 0,  maxRemain = 9,  minStack = 0 },
  [325876]  = { minRemain = 0 },
  [346844]  = { minRemain = 2 },
  [349954]  = { minRemain = 0 },
  [351960]  = { minRemain = 2, minStack = 2 },
  [355641]  = { minRemain = 2 },
  [355915]  = { minRemain = 2 },
  [356943]  = { minRemain = 2 },
  [426735]  = { minRemain = 0 },
  ...
  [448515]  = { minRemain = 2 },
  [451606]  = { minRemain = 2 },
  [462737]  = { minRemain = 3, minStack = 4 },
  [469799]  = { minRemain = 0 },
  [1221484]  = { minRemain = 2 },
  [1224977]  = { minRemain = 0, minStack = 7 },
  [1227052]  = { minRemain = 2 },
  [1228161]  = { minRemain = 2 },
  [1235060]  = { minRemain = 2,  minStack = 7 },
  [1235762]  = { minRemain = 2 },
  [1236513]  = { minRemain = 2 },
  [1238834]  = { minRemain = 2,  minStack = 2 },
  [1241785]  = { minRemain = 2, minStack = 5 },
  [438618]  = { minRemain = 2 },
  [432227]  = { minRemain = 2 },
  [448248]  = { minRemain = 2 },
  [465813]  = { minRemain = 2 },
  ...
  -- Example with a custom predicate:
  -- [SOME_ID] = { maxRemain = 10, minStack = 2, role = "ANY", pred = function(u, aura) return Unit(u):HealthPercent() < 80 end },
}
local DispelRules2 = {
  [350101] = { minRemain = 0 }, 
  [351960]  = { minRemain = 2,  minStack = 2 },
  ...
  [431309]  = { minRemain = 2 },
  [433662]  = { minRemain = 2 },
  [434830]  = { minRemain = 0, minStack = 2 },
  [442210]  = { minRemain = 2 },
  [453461]  = { minRemain = 2 },
  [465813]  = { minRemain = 2 },
  [468672]  = { minRemain = 2, minStack = 2 },
  [1213803]  = { minRemain = 1 },
  [1220671]  = { minRemain = 0 },
  [1235245]  = { minRemain = 2, minStack = 2 },
...
local DispelRules4 = {
  ...
  [347481]  = { minRemain = 2 },
  [431333]  = { minRemain = 2 },
  [433747]  = { minRemain = 0, minStack = 2 },
  [446403]  = { minRemain = 2, minStack = 2 },
  [448787]  = { minRemain = 2 },
  [451107]  = { minRemain = 2 },
  [451119]  = { minRemain = 2 },  
...
-- ==== generic rule checker using Action's Unit() helpers ====
local function _passesRule_Action(unit, spellId, rule)
    -- remain and total duration (seconds)
    local remain, duration = Unit(unit):SortDeBuffs(spellId, nil, true)  -- byID=true
    if not remain or remain <= 0 then
        return false
    end

    -- stacks (applications) ; normalize 0 => 1 like Blizzard API does for unstacked auras
    local stacks = Unit(unit):HasDeBuffsStacks(spellId, nil, true)
    if stacks == 0 then stacks = 1 end

    -- role gate
    if rule.role then
        if rule.role == "HEALER" and not Action.IamHealer then return false end
        if rule.role == "DAMAGER" and Action.IamHealer then return false end
        -- "ANY" passes
    end

    -- time + stacks gates
    if rule.minRemain and remain <  rule.minRemain then return false end
    if rule.maxRemain and remain >= rule.maxRemain then return false end
    if rule.minStack  and stacks <  rule.minStack  then return false end

    -- optional predicate (note: debuffType isn't available through these wrappers;
    -- pass nil for it unless you maintain a side table)
    if rule.pred and not rule.pred(unit, {
        spellId    = spellId,
        count      = stacks,
        remain     = remain,
        duration   = duration,
...
local function _checkRulesForUnit(unit, rules)
    for spellId, rule in pairs(rules) do
        if _passesRule_Action(unit, spellId, rule) then
            return true
        end
    end
    return false
...
-- === public helpers ===
local function MyCanCleanse(unit)
    if Unit(unit):IsDead() then return false end
    if Unit(unit):HasDeBuffs(A.Cyclone.ID, nil, true) > 0 then return false end
    return _checkRulesForUnit(unit, DispelRules)
end

local function MyCanBoF(unit)
    if Unit(unit):IsDead() then return false end
    if Unit(unit):HasDeBuffs(A.Cyclone.ID, nil, true) > 0 then return false end
    return _checkRulesForUnit(unit, DispelRules2)
end

local function MyCanBoP(unit)
    if Unit(unit):IsDead() then return false end
    if Unit(unit):HasDeBuffs(A.Cyclone.ID, nil, true) > 0 then return false end
    return _checkRulesForUnit(unit, DispelRules3)
end

local function MyCanBoS(unit)
    if Unit(unit):IsDead() then return false end
    if Unit(unit):HasDeBuffs(A.Cyclone.ID, nil, true) > 0 then return false end
    return _checkRulesForUnit(unit, DispelRules4)
end
...
--========================================================
-- (12) DANGEROUS CAST SETS + WRAPPER (with QoL upgrades)
--========================================================
local DANGEROUS_VERSION = "TWW_S3"

-- Trigger immediately while casting/channeling
local DANGEROUS_ANY = {
    [346742]=true,[355429]=true,[357243]=true,[357508]=true,
    [423015]=true,[423665]=true,
    [426787]=true,[428169]=true,
    [438476]=true,
    [448492]=true,
    [448888]=true,[460156]=true,
}

-- Trigger only when remaining cast time <= threshold
-- value = true  -> uses default threshold (1.0s)
-- value = 0.6   -> custom threshold in seconds
local DANGEROUS_LATE = {
    [326409]=true,[328791]=true,[350875]=true,[424431]=true,[427609]=true,
    [448791]=true,[453212]=true,[465827]=true,[465463]=true,[473070]=true,
    [1217232]=true,[1219700]=true,[1221532]=true,[1241693]=true,
}

-- === Latency cushions ===
local function PingSec() return A.GetPing() or 0 end

local DEFAULT_LATE_THRESHOLD = 1.0                -- exact 1.0s base
local RECENT_DANG_WINDOW     = 0.5 + PingSec()    -- post-cast cushion
local LAST_DANG_LATE_T       = 0

-- === CLEU stamping for "recent finished late cast" window ===
do
    ...
    f:SetScript("OnEvent", function()
        ...
        if subEvent == "SPELL_CAST_SUCCESS"
           and DANGEROUS_LATE[spellID] ~= nil
           and bit_band(sourceFlags or 0, COMBATLOG_OBJECT_REACTION_HOSTILE) ~= 0
        then
            LAST_DANG_LATE_T = GetTimePreciseSec()
        end
    end)
end

-- === Lookup helpers ===
local function IsDangerousAny(spellID)
    return spellID and DANGEROUS_ANY[spellID] == true
end

local function LateThresholdFor(spellID)
    local v = spellID and DANGEROUS_LATE[spellID]
    if v == nil then return nil end
    if v == true then return DEFAULT_LATE_THRESHOLD end
    return v -- numeric seconds
end

-- === MAIN CHECK ===
-- Uses Action.Unit():IsCastingRemains to get seconds remaining cleanly
local function CheckEnemyCasting_cached()
    local now = GetTimePreciseSec()

    -- 0) Recently-finished late-danger cast (keep window briefly open)
    if (now - LAST_DANG_LATE_T) <= RECENT_DANG_WINDOW then
        return true
    end

    -- 1) Scan current enemy nameplates
    for i = 1, #ActiveUnits do
        local unit = ActiveUnits[i]
        if Unit(unit):IsEnemy() then
            -- [1]=leftSec, [2]=leftPct, [3]=spellID
            local leftSec, _, spellID = Unit(unit):IsCastingRemains(nil)
            if leftSec and leftSec > 0 and spellID then

                -- Immediate bucket
                if IsDangerousAny(spellID) then
                    return true
                end

                -- Late bucket
                local thr = LateThresholdFor(spellID)
                if thr and leftSec <= thr then
                    LAST_DANG_LATE_T = now
                    return true
                end

                -- (Optional: more accurate to server)
                -- if thr and (leftSec - PingSec()) <= thr then ...
            end
        end
    end
...
A[2] = function(icon)
    ...
    if left and left > A.GetPing() then
        if not notKickAble
        and A.Rebuke:IsReadyByPassCastGCD(unitID, nil, true, true)
...
A[3] = function(icon, isMulti)
    ...
    local function HealingRotation(unitID) 
		...
		local function ShouldBlindingLight()
			local now = GetTime()
			local gcd = (A.GetGCD and A.GetGCD()) or 1.5
			local timeThreshold = math.max(gcd * 0.8, 0.6) -- tolerate latency + update lag

			for i = 1, #ActiveUnits do
				local u = ActiveUnits[i]

				if Unit(u):IsEnemy()
				   and not Unit(u):IsBoss()
				   and Unit(u):IsControlAble("disorient", 25) -- Blinding Light’s CC type
				   and not CTT_ShouldAvoidStun(u)
				   and A.CrusaderStrike:IsInRange(u) -- reliable ~10y check
				then
					-- Prefer Action helper (handles channels/casts)
					local leftSec = select(1, Unit(u):IsCastingRemains(nil))
					if not leftSec or leftSec <= 0 then
						local cc = CastCache[u]
						if cc then
							leftSec = math.max(0, (cc[2] or now) - now)
						end
					end

					if leftSec and leftSec >= timeThreshold then
						return true
					end
				end
			end
        ...
        if A.Rebuke:IsReadyByPassCastGCD("target") and Unit("target"):CanInterrupt(true, nil, 20, 70) and A.Rebuke:AbsentImun("target", Temp.TotalAndPhysKick, true) then
        ...
        if A.Rebuke:IsReadyByPassCastGCD("mouseover")
        and combatTime > 0
        ...
        do
            for i = 1, #membersAll do 
                if combatTime > 0 and A.BlessingofProtection:IsInRange(membersAll[i].Unit) and (A.InstanceInfo and A.InstanceInfo.KeyStone or 0) >= 15 then 
        ...
        if Unit(unitID):HealthPercent() >= 20 then
            if GetTime() >= nextMassUtils then
				...
				if A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
					local moExists = Unit("mouseover"):IsExists()
					for i = 1, #membersAll do
						local u = membersAll[i].Unit
						local U = Unit(u)
						if A.Cleanse:IsInRange(u)
						and (not moExists or UnitIsUnit("mouseover", u))   -- only if no mouseover, or mouseover == this unit
						and MyCanCleanse(u)
						and U:HasDeBuffs(342938) == 0 then
							HealingEngine.SetTarget(u, 0.5)
				...
				if A.BlessingofFreedom:IsReady() and A.GetToggle(2, "DispelSniper") then
					...
					for i = 1, #membersAll do
						local u = membersAll[i].Unit
						if A.BlessingofFreedom:IsInRange(u)
						and (not moExists or UnitIsUnit("mouseover", u))   -- only if no mouseover, or mouseover == this unit
						and CanUseFreedomNow()
						and not IsCastingSpell("target", 432117)
						and Unit(u):HasBuffs(A.BlessingofFreedom.ID) == 0
						and MyCanBoF(u) then
							HealingEngine.SetTarget(u, 0.5)
							return A.BlessingofFreedom:Show(icon)
						end
                ...
                if A.BlessingofProtection:IsReady() and A.GetToggle(2, "DispelSniper") then
                    for i = 1, #membersAll do
                        ...
                        and MyCanBoP(membersAll[i].Unit)
                        and not U:Role("TANK") then
                            HealingEngine.SetTarget(membersAll[i].Unit, 0.5)
							return A.BlessingofProtection:Show(icon)
                        end
                    end
                ...
                if A.BlessingofSacrifice:IsReadyByPassCastGCD() and A.GetToggle(2, "DispelSniper") then
                    for i = 1, #membersAll do
                        ...
                        and MyCanBoS(membersAll[i].Unit)
                        and not UnitIsUnit(membersAll[i].Unit, player) then
                            HealingEngine.SetTarget(membersAll[i].Unit, 0.5)
							return A.BlessingofSacrifice:Show(icon)
                        end
                    end
            ...
            if A.Cleanse:IsReadyP(mouseover) and A.GetToggle(2, "MouseoverDispel") and A.MouseHasFrame()
