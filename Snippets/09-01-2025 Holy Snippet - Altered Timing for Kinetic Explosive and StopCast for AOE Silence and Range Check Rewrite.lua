...
A[3] = function(icon, isMulti)
	...
	local function DoNotCastFOL()
		...
		for i = 1, 40 do
			...
			if UnitExists(unitID) and IsUnitEnemy(unitID, "player") then
				local _, _, _, _, endMS, _, _, _, spellID = UnitCastingInfo(unitID)
				if not spellID then
					_, _, _, _, endMS, _, _, spellID = UnitChannelInfo(unitID)
				end

				if spellID and watch[spellID] then
					local enemyRem = Unit(unitID):IsCastingRemains() 
									 or (endMS and math.max(0, (endMS - GetTime() * 1000) / 1000)) 
									 or 0
					local folExec  = Player:Execute_Time(A.FlashofLight.ID) or 0
					return folExec > enemyRem
				end
			end
		end
	...
	local function DoNotCastHL()
		...
		for i = 1, 40 do
			...
			if UnitExists(unitID) and IsUnitEnemy(unitID, "player") then
				...
				if spellID and watch2[spellID] then
					local enemyRem = Unit(unitID):IsCastingRemains() 
									 or (endMS and math.max(0, (endMS - GetTime() * 1000) / 1000)) 
									 or 0
					local folExec  = Player:Execute_Time(A.HolyLight.ID) or 0
					return folExec > enemyRem
				end
			end
		end
    ...
    local function HealingRotation(unitID) 
        ...
        then
            for i = 1, #getmembersAll do 
                if A.LayOnHands:IsInRange(getmembersAll[i].Unit) then 
        ...
        if GetToggle(1, "StopCast") then
            ...
            if (DoNotCastHL() and Unit(player):IsCasting(A.HolyLight.ID)) or (DoNotCastFOL() and Unit(player):IsCasting(A.FlashofLight.ID)) then
        ...
        for i = 1, #getmembersAll do 
            if combatTime > 0 and useShields and A.BlessingofProtection:IsInRange(getmembersAll[i].Unit) and A.InstanceInfo.KeyStone >= 15 then 
        ...
        and not IsUnitEnemy(unitID) and A.DivineToll:IsInRange(unitID) and Unit(unitID):HealthPercent() < HolyShockHP and
        ...
        if not Action.InstanceInfo.isRated or Action.InstanceInfo.isRated and (Unit(unitID):HealthPercent() >= 30 or Unit(unitID):HealthPercent() < 30 and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady()) then
            ...
            if (A.Zone == "arena" or A.InstanceInfo.isRated) and A.BlessingofFreedom:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if A.BlessingofFreedom:IsInRange(getmembersAll[i].Unit) and not Unit(getmembersAll[i].Unit):IsDead() and Unit(getmembersAll[i].Unit):HasBuffs(A.BlessingofFreedom.ID) == 0 
            ...
            if A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if A.Cleanse:IsInRange(getmembersAll[i].Unit) 
					and
						(
						...
						and
						(Unit(getmembersAll[i].Unit):HasDeBuffs(473713) == 0 or Unit(getmembersAll[i].Unit):HasDeBuffs(473713) < 9)	
						)					
            ...
            if A.BlessingofFreedom:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if A.BlessingofFreedom:IsInRange(getmembersAll[i].Unit) and Unit(getmembersAll[i].Unit):GetCurrentSpeed() <= 70 and not Unit(getmembersAll[i].Unit):IsDead() and not HoldFreedom() and not Unit(target):IsCasting(432117) and Unit(getmembersAll[i].Unit):HasBuffs(A.BlessingofFreedom.ID) == 0 
            ...
            if A.BlessingofProtection:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if A.BlessingofProtection:IsInRange(getmembersAll[i].Unit) and not Unit(getmembersAll[i].Unit):IsDead() and Unit(getmembersAll[i].Unit):HasBuffs(A.BlessingofProtection.ID) == 0 and Unit(getmembersAll[i].Unit):HasDeBuffs(A.Forbearance.ID) == 0
            ...
            if A.BlessingofSacrifice:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if A.BlessingofSacrifice:IsInRange(getmembersAll[i].Unit) and not Unit(getmembersAll[i].Unit):IsDead() and Unit(getmembersAll[i].Unit):HasBuffs(A.BlessingofSacrifice.ID) == 0 and Unit(getmembersAll[i].Unit):HasBuffs("TotalImun") == 0
            ...
            if A.Cleanse:IsReady(unitID) 
						...
						and
						(Unit(unitID):HasDeBuffs(473713) == 0 or Unit(unitID):HasDeBuffs(473713) < 9)
						)
        ...
        if Unit(target):HasDeBuffs("BreakAble") == 0 and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 then
            ...
            if A.Judgment:IsReady(target) and IsUnitEnemy(target) and Unit(target):HasDeBuffs(A.Cyclone.ID, nil, true) == 0 and (Unit(target):CombatTime() > 0 or A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp") and ((not IsInGroup() or not IsInRaid()) or ((IsInGroup() or IsInRaid()) and (Unit(player):HasBuffs(A.AwakeningBuff.ID) == 0 or (Unit(player):HasBuffs(A.AwakeningBuff.ID) <= 5 or (Unit(player):HasBuffs(A.AwakeningBuff.ID) > 5 and HealingEngine.GetBelowHealthPercentUnits(80, 40) >= 4) and Unit(player):HasBuffs(A.AvengingWrath.ID) == 0)))  
    ...
    local function FillerRotation(unitID) 
		...
		if not inCombat or inCombat and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady() then 
			...
			if not DoNotCastHL() and A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.HolyLight.ID) and not A.WordofGlory:IsReady(unitID) 
			...
			if not DoNotCastFOL() and Player:ManaPercentage() > GetToggle(2, "ManaConservation") and A.FlashofLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.FlashofLight.ID) 
			...
			if not DoNotCastHL() and Player:ManaPercentage() > GetToggle(2, "ManaConservation") and A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.HolyLight.ID) and not A.WordofGlory:IsReady(unitID) 
			and Player:Execute_Time(A.HolyLight.ID) - A.HolyShock:GetCooldown() <= Player:Execute_Time(A.HolyLight.ID)/2 and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() 
    ...
    local function DamageRotation(unitID)
        ...
        if A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp" or inCombat then
			...
			-- Usage inside APL:
			if useCC and A.BlindingLight:IsReady("player") and A.HammerofJustice:IsInRange(target) and not HoldFreedom() and ShouldBlindingLight() then
				return A.BlindingLight:Show(icon)
			end
            ...
            if (Unit(unitID):HasDeBuffs("BreakAble") == 0 and (A.Zone == "arena" or A.InstanceInfo.isRated)) or (not (A.Zone == "arena" or A.InstanceInfo.isRated) and ((inCombat and Unit(target):CombatTime() > 0) or TeamCacheFriendlySize == 1)) then
